...
static  __global__ void im2col_kernel(
    ...
    const int64_t n = N_OH * OW * IC_KH_KW;
    const int64_t stride = (int64_t) blockDim.x * gridDim.x;

    for (int64_t idx = (int64_t) blockIdx.x * blockDim.x + threadIdx.x; idx < n; idx += stride) {
        const int64_t i = idx % IC_KH_KW;
        const int64_t patch = idx / IC_KH_KW;
        const int64_t iow = patch % OW;
        const int64_t iz = patch / OW;
        ...
        if (iih < 0 || iih >= IH || iiw < 0 || iiw >= IW) {
            ...
            dst[idx] = x[offset_src + iih * IW + iiw];
        }
    }
...
static void im2col_cuda(const float * x, T* dst,
    ...
    const int64_t n = N_OH * OW * IC_KH_KW;
    const int64_t num_blocks = MIN((n + CUDA_IM2COL_BLOCK_SIZE - 1) / CUDA_IM2COL_BLOCK_SIZE, 65535);
    im2col_kernel<<<num_blocks, CUDA_IM2COL_BLOCK_SIZE, 0, stream>>>(x, dst, IC, IW, IH, OH, OW, KW, KH,
